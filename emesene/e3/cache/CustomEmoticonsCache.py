''' a class that holds a map of shortcuts:custom emoticons '''
...
class CustomEmoticonsCache(Cache.Cache):
    ''' a class to maintain a cache of custom emoticons '''
    ...
    def __init__(self, config_path, user):
        ...
        Cache.Cache.__init__(self, os.path.join(config_path,
            user.strip()), 'customemoticons', True)

    def add_custom_emoticon(self, path, shortcut):
    ''' add a new entry '''

    def del_custom_emoticon(self, shortcut):
    ''' delete a custom emoticon '''
    
    def mod_custom_emoticon(self, path, shortcut):
    ''' modify existing path/shortcut '''
