...
class CellRendererPlus(gtk.GenericCellRenderer):
    ...
    def get_layout(self, widget):
        ...
        if self.markup:
            plused_markup = Plus.msnplus(self.markup).to_xml()
            layout.set_markup(plused_markup)
        else:
            layout.set_text('')
...
@extension.implements('nick renderer')
class CellRendererNoPlus(gtk.GenericCellRenderer):
    __gproperties__ = {
            'markup': (gobject.TYPE_STRING,
                "text",
                "text we'll display (even with plus markup!)",
    ...
    def get_layout(self, widget):
        ...
        layout.set_width(-1)    # Do not wrap text.
        if self.markup:
            noplused_markup = Plus.msnplus_strip(self.markup)
            layout.set_markup(noplused_markup)
        else:
            layout.set_text('')
